...
int Height = size * (M + Space);				//Height (h) shows the number of pixel available (= size x (M + Space)      
...
//Structing class
struct snake {int x, y;}  Snake[10000], Snake_2[10000];      	//Make object named Snake with two data shows position (x, y) and set it as array
struct Fruit {int x, y;}  Food, FoodMode[150]; int FoodChoice, FoodStatus[150]; int FoodModeCount = 1; float FoodModeDuration, FoodModeRealTime; 	//Make object named Food with two data shows position (x, y)
...
Texture t12, t100; Sprite sprite100(t100); int RestartStatus = 0;
float RealTime;
...
void EasyMode ()
{
	...
	if ((Snake[0].x == Food.x) && (Snake[0].y == Food.y)) 
    {
		...
		if (FoodChoice == 2) {score = score + 6;  Snake_Length = Snake_Length + 3; }		//Melon, Increment the Snake's Length by 3
...
void Restart() 	
{
	...
	MusicChoice = rand() % 6 + 1; MusicPlay(); RealTime = 0.0;
}
...
{  
    ...
    while (window.isOpen())                       					 //Running the file and keep open
    {
		...
		FoodModeRealTime = FoodModeRealTime + time;					//Increment the 'FoodModeRealTime' by time
	...
	// ----- DRAWING THE WINDOW ----//
		...
		// --- Create Bonus Points ---
		Text text30; Font font1; font1.loadFromFile("Images/Times-New-Romans.ttf");
		if (FoodModeRealTime > 75)
		{
			text30.setString("BONUS TIME - DEATHLY TIME");	
			if (FoodModeCount == 1)
			{
				for (int i = 0; i < 150; i++)
				{		
					FoodStatus[i] = rand() % 3 + 1; 
					FoodMode[i].x = rand() % N; 
					FoodMode[i].y = rand() % M + Space;  
				}
			}
			for (int i = 0; i < 150; i++) 
			{	
				if (FoodStatus[i] == 1) {t100.loadFromFile("Images/Fruit/Apple.png"); 		Sprite sprite100(t100); sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100); }  	//Apple
				if (FoodStatus[i] == 2) {t100.loadFromFile("Images/Fruit/Melon.png"); 		Sprite sprite100(t100); sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100); }	//Melon
				if (FoodStatus[i] == 3) {t100.loadFromFile("Images/Fruit/Strawberry.png"); 	Sprite sprite100(t100); sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100); } 	//Strawberry
				if ( (Snake[0].x == FoodMode[i].x) and (Snake[0].y == FoodMode[i].y) )
				{
					if ( FoodStatus[i] == 1) {score = score + 10; Snake_Length = Snake_Length + 5;  FoodMode[i].x = -1; FoodMode[i].y = -1; }
					if ( FoodStatus[i] == 2) {score = score + 20; Snake_Length = Snake_Length + 10; FoodMode[i].x = -1; FoodMode[i].y = -1; }
					if ( FoodStatus[i] == 3) {score = score - 50; Snake_Length = Snake_Length - 25; FoodMode[i].x = -1; FoodMode[i].y = -1; }
					sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100);
				}
			}
			FoodModeCount = 0; FoodModeDuration = FoodModeDuration + time; 
			if (FoodModeDuration > 15)	
			{
				for (int i = 0; i < 150; i++)
				{		
					FoodMode[i].x = -1 ; FoodMode[i].y = -1 ;
					sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100);
				}
				FoodModeRealTime = 0; FoodModeDuration = 0; FoodModeCount = 1; 
				text30.setString("");
			}
			text30.setFont(font1); 
			text30.setCharacterSize(24); 	text30.setFillColor(Color::Red);
			text30.setStyle(Text::Bold); 	text30.setPosition(820, 4); 
			window.draw(text30);
		}	
	
		//Print the text 
		Text text0, text1, text2, text3, text6, text8, text9; font.loadFromFile("Images/Times-New-Romans.ttf");
		...
		text8.setFont(font); 			text8.setString("Time Played (s): ");
		text8.setCharacterSize(24); 	text8.setFillColor(Color::Red);
		text8.setStyle(Text::Bold); 	text8.setPosition(900, 30); 
		window.draw(text8);
		char TimePlayed[10000]; 		itoa(RealTime, TimePlayed, 10); 			//convert 'RealTime' (Time Played) from char to string
		text9.setFont(font); 			text9.setString(TimePlayed);
		text9.setCharacterSize(24); 	text9.setFillColor(Color::White);
		text9.setStyle(Text::Bold); 	text9.setPosition(1090, 30); 
		window.draw(text9);
...
void SingleBrutal()
{
	...
	// ----- DRAWING THE WINDOW ----//
		...
		if (ObstacleRealTime > 15)
		{
			...
			for (int i = 0; i < ObstacleNumber; i++) {	sprite0.setPosition( Obstacle[i].x*size, Obstacle[i].y*size); window.draw(sprite0); }  
			ObstacleCount = 0; ObstacleDuration = ObstacleDuration + time;
			if (ObstacleDuration > 45)	{ObstacleRealTime = 0; ObstacleDuration = 0; ObstacleCount = 1; ObstacleDelete(); ObstacleNumber = ObstacleNumber * 1.75;} //45 seconds on and off
		}
		// --- Create Bonus Points ---
		Text text30; Font font1; font1.loadFromFile("Images/Times-New-Romans.ttf");
		if (FoodModeRealTime > 75)
		...
		text6.setFont(font); 			text6.setString("Game Mode: Single Brutal - Status: ");
		text6.setCharacterSize(16); 	text6.setFillColor(Color::White);
		text6.setStyle(Text::Bold); 	text6.setPosition(2, 44); 
		window.draw(text6);	
		text7.setFont(font); 			
		text7.setCharacterSize(16); 	text7.setFillColor(Color::White);
		text7.setStyle(Text::Bold); 	text7.setPosition(280, 44); 
		window.draw(text7);
...
void EasyMode_2 ()
{
	...
	if ((Snake_2[0].x == Food.x) && (Snake_2[0].y == Food.y)) 
    {
		...
		if (FoodChoice == 2) {score_2 = score_2 + 6;  Snake_Length_2 = Snake_Length_2 + 3;}			//Melon, Increment the Snake's Length by 3
...
void DoubleEasy()
{
	...
	// ----- DRAWING THE WINDOW ----//
		...
		if (FoodModeRealTime > 75)
		{
			...
			{	
				...
				if ( (Snake_2[0].x == FoodMode[i].x) and (Snake_2[0].y == FoodMode[i].y) )
				{
					if ( FoodStatus[i] == 1) {score_2 = score_2 + 10; Snake_Length_2 = Snake_Length_2 + 5;  FoodMode[i].x = -1; FoodMode[i].y = -1; }
					if ( FoodStatus[i] == 2) {score_2 = score_2 + 20; Snake_Length_2 = Snake_Length_2 + 10; FoodMode[i].x = -1; FoodMode[i].y = -1; }
					if ( FoodStatus[i] == 3) {score_2 = score_2 - 50; Snake_Length_2 = Snake_Length_2 - 25; FoodMode[i].x = -1; FoodMode[i].y = -1; }
					sprite100.setPosition(FoodMode[i].x * size, FoodMode[i].y * size); window.draw(sprite100);
				}
			}
			FoodModeCount = 0; FoodModeDuration = FoodModeDuration + time; 
	...
	while (window1.isOpen())
	{
		...
		text12.setCharacterSize(40); 	text12.setFillColor(Color::Red);
...
void DoubleHard()
{
	...
	// ----- DRAWING THE WINDOW ----//
		...
		for (int i = 1; i < Snake_Length_2; i++) 
    	{
    		//Hiting the Snake #2 himself
...
void DoubleBrutal()
{
	...
	// ----- DRAWING THE WINDOW ----//
		...
		if (ObstacleRealTime > 15)
		{
			...
			for (int i = 0; i < ObstacleNumber; i++) {	sprite0.setPosition( Obstacle[i].x*size, Obstacle[i].y*size); window.draw(sprite0); }  
			ObstacleCount = 0; ObstacleDuration = ObstacleDuration + time;
			if (ObstacleDuration > 45)	{ObstacleRealTime = 0; ObstacleDuration = 0; ObstacleCount = 1; ObstacleDelete(); ObstacleNumber = ObstacleNumber * 1.75; } //20 seconds on and off
		}
		// --- Create Bonus Points ---
		Text text30; Font font1; font1.loadFromFile("Images/Times-New-Romans.ttf");	
		if (FoodModeRealTime > 75)
		...
		text6.setFont(font); 			text6.setString("Game Mode: Double Brutal - Status: ");
		text6.setCharacterSize(16); 	text6.setFillColor(Color::White);
		text6.setStyle(Text::Bold); 	text6.setPosition(2, 44); 
		window.draw(text6);	
		text7.setFont(font); 			
		...
		while (window0.pollEvent(e))
		{
			...
			if (e.type == Event::KeyPressed)
			{
				...
				{ 
					...
					int n = playerInput.getSize() - 1;
					for (int i = 0; i < n; i++) {Player[PlayerNumber].name[i] = playerInput[i];}
					Player[PlayerNumber].name[n] = '_'; Player[PlayerNumber].name[n+1] = 'S'; Player[PlayerNumber].name[n+2] = 'B';
					Restart(); window0.close(); SingleBrutal(); RestartStatus = 0; Restart();	
				}
				if (Keyboard::isKeyPressed(Keyboard::D) and Keyboard::isKeyPressed(Keyboard::E)) 
				{ 
					//Name Conversion
				...
				{ 
					//Name Conversion
				 	int n = playerInput.getSize() - 1; 
					for (int i = 0; i < n; i++) {Player[PlayerNumber].name[i] = playerInput[i];}
					Player[PlayerNumber].name[n] = '_'; Player[PlayerNumber].name[n+1] = 'D'; Player[PlayerNumber].name[n+2] = 'B';
					Restart(); window0.close(); DoubleBrutal(); RestartStatus = 0; Restart();
				}
			}
		}
